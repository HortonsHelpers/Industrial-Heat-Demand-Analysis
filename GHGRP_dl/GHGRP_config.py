...
EFs = pd.DataFrame(EFs[EFs.dup_index == False], columns=EFs.columns[:2])
...
aa_sl_file = f"aa_sl_table_{str(years[0])}{str(years[-1])}.csv"

aa_sl_table.to_csv(file_dir + aa_sl_file)

aa_ffuel_file = f"aa_ffuel_table_{str(years[0])}{str(years[-1])}.csv"
...
facfiles_201115 = [file_dir + "fac_table_" + str(y) + ".csv" for y in years]
